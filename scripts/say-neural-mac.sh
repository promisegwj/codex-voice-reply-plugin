  ...
  while [[ "$directory" != "/" && -n "$directory" ]]; do
    for settings_file in "voice-project-settings.json" ".codex-voice.json"; do
      ...
      if [[ -f "$settings_path" ]]; then
        if grep -Eq '"suppressAllSpeech"[[:space:]]*:[[:space:]]*true|"strategy"[[:space:]]*:[[:space:]]*"voice_disabled"|"modeOverride"[[:space:]]*:[[:space:]]*"voice_disabled"' "$settings_path"; then
          return 0
        fi
      fi
    done
...
if [[ "$IGNORE_PROJECT_VOICE_SETTINGS" -eq 0 && "$PROFILE_EXPLICIT" -eq 0 ]]; then
  if resolved_profile="$(settings_voice_profile "$PROJECT_ROOT")"; then
