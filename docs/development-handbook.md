...
2. 增加一个 `resolve-voice-strategy` 小工具，用于输出当前项目最终会不会发声、使用哪个模式、哪个声音。
3. 给 `voice-interaction-modes.json` 和 `project-voice-strategies.json` 增加 JSON Schema。
4. 提供一组更短、更适合 release assets 的试听样本。
5. 增加自动化验证脚本，统一跑 JSON、PowerShell、隐私扫描和基础语音生成。
6. 进一步整理 Codex plugin 的 marketplace 说明和截图/示例。
